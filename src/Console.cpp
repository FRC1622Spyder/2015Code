...
bool Spyder::Console::Connect(const std::string &strIP, unsigned short usPort)
{
	...
	m_serverAddr.sin_addr.s_addr = inet_addr(const_cast<char*>(strIP.c_str()));*/
	return false;
}
...
bool Spyder::Console::SendPacket(const std::string &strSubsystem, Packet& packet)
{
	...
	ss << strSubsystem;
	ss << packet.GetData();
	if(sendto(m_socket, const_cast<char*>(ss.str().c_str()), ss.str().length(), 0, (sockaddr*)&m_serverAddr, sizeof(sockaddr_in)) == -1)
	{
		return false;
	}
	return true;
